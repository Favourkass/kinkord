      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <ul>
          {conversations.map((c) => {
            ...
            const preview =
              c.lastMessage?.body ??
              (c.lastMessage?.attachments?.length ? "📎 Attachment" : "No messages yet");
            return (
              <li key={c.id}>
                <button
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-medium truncate">
                        {peer?.displayName ?? c.title ?? "Conversation"}
                      </div>
                      {c.unreadCount > 0 && (
                    ...
                    <div
                      className={`text-xs truncate ${c.unreadCount > 0 ? "text-slate-900 font-medium" : "text-slate-500"}`}
                    >
                      {preview}
                    </div>
                  </div>
