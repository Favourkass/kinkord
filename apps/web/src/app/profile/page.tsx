...
function OwnProfileView({ username }: { username: string }) {
  const search = useSearchParams();
  const shell = useHomePresenter();
  const nav = getAppShellNav();
  const vm = useMemberProfilePresenter(username, search.get("tab"));
...
export default function ProfilePage() {
  ...
  return <OwnProfileView username={p.me.username} />;
}
