const shell = (title: string, body: string, ctaLabel: string, ctaUrl: string) => `<!doctype html>
<html>
...
export const verificationEmail = (url: string) => ({
  ...
  html: shell(
    ...
    "<p>Welcome to Kinkord. Confirm this email address to activate your account.</p>",
    "Verify email",
    url,
  ),
...
export const resetPasswordEmail = (url: string) => ({
  ...
  html: shell(
    "Reset your password",
    "<p>We received a request to reset your password. This link expires shortly. If it wasn't you, ignore this email — your password stays unchanged.</p>",
    "Reset password",
    url,
  ),
