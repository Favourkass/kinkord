...
1. Start local PostgreSQL and MinIO using `docker compose up -d postgres minio minio-init`, then run `corepack pnpm --filter api db:migrate` for migration `0008`.
